...
public class QuoineAdapters {
  ...
  public static Wallet adaptWallet(QuoineAccountBalance[] quoineWallet) {

	    List<Balance> balances = new ArrayList<>();

	    for(QuoineAccountBalance q:quoineWallet){	    	
	    	Balance b =new Balance(Currency.getInstance(q.getCurrency()), BigDecimal.valueOf(q.getBalance()));
	    	balances.add(b);
	    }	    
	    return new Wallet(balances);
